...
/**
 * 实体[工作流步骤子实例]搜索表单[searchform]部件模型
 */
public class  WFStepInstDefaultSearchFormModel extends net.ibizsys.paas.ctrlmodel.SearchFormModelBase {

    public WFStepInstDefaultSearchFormModel() {
        super();
    ...
    protected void prepareFormItems()throws Exception {
        ...
        // n_wfstepinstname_like
        iFormItem = this.createFormItem("n_wfstepinstname_like");
        if(iFormItem==null) {
            SearchFormItemModel formItem = new SearchFormItemModel();
            formItem.setForm(this);
            formItem.setName("n_wfstepinstname_like");
            formItem.setDEFName("WFSTEPINSTNAME");
            formItem.setCaption("工作流步骤子实例名称(%)");
            //设置数据项参数
            DataItemModel dataItem = new DataItemModel();
            dataItem.setName("n_wfstepinstname_like");
