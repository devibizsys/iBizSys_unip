...
/**
 * 实体[用户角色数据操作]编辑表单[form]部件模型
 */
public class UserRoleDataActionMainEditFormModel extends net.ibizsys.paas.ctrlmodel.EditFormModelBase {

    public UserRoleDataActionMainEditFormModel()  {
        super();
        this.setName("form");
    }
    ...
    protected void prepareFormItems()throws Exception {
        ...
        if(iFormItem==null) {
            ...
            formItem.setDEFName("USERROLEDATAACTIONID");
            formItem.setCaption("用户角色数据操作标识");
        ...
        if(iFormItem==null) {
            ...
            formItem.setEnableCond(1);
            formItem.setCaption("用户角色数据操作_用户角色数据");
        ...
        this.registerFormItem(iFormItem);
        // userroledataactionname
        iFormItem = this.createFormItem("userroledataactionname");
        if(iFormItem==null) {
            EditFormItemModel formItem = new EditFormItemModel();
            formItem.setForm(this);
            formItem.setName("userroledataactionname");
            formItem.setDEFName("USERROLEDATAACTIONNAME");
            formItem.setEnableCond(1);
            formItem.setCaption("操作名称");
            ...
            //设置数据项参数
            DataItemModel dataItem = new DataItemModel();
            dataItem.setName("userroledataactionname");
            dataItem.setDataType(25);
            dataItem.setFormat("%1$s");
            formItem.setDataItem(dataItem);
            formItem.init();
        ...
        this.registerFormItem(iFormItem);
        // isallow
        iFormItem = this.createFormItem("isallow");
        if(iFormItem==null) {
            EditFormItemModel formItem = new EditFormItemModel();
            formItem.setForm(this);
            formItem.setName("isallow");
            formItem.setDEFName("ISALLOW");
            formItem.setCodeListId("com.sa.unip.srv.codelist.YesNoCodeListModel");
            formItem.setCaption("允许");
            formItem.setAllowEmpty(false);
            formItem.setOutputCodeListConfig(true);

            //设置数据项参数
            DataItemModel dataItem = new DataItemModel();
            dataItem.setName("isallow");
        ...
        this.registerFormItem(iFormItem);
        // userroledataactionid
        iFormItem = this.createFormItem("userroledataactionid");
        if(iFormItem==null) {
            EditFormItemModel formItem = new EditFormItemModel();
            formItem.setForm(this);
            formItem.setName("userroledataactionid");
            formItem.setDEFName("USERROLEDATAACTIONID");
            formItem.setCaption("用户角色数据操作标识");

            //设置数据项参数
            DataItemModel dataItem = new DataItemModel();
            dataItem.setName("userroledataactionid");
            dataItem.setDataType(25);
            dataItem.setFormat("%1$s");
            formItem.setDataItem(dataItem);
            formItem.init();
