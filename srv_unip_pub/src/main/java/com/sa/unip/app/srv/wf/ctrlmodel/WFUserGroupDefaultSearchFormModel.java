...
/**
 * 实体[工作流用户组]搜索表单[searchform]部件模型
 */
public class  WFUserGroupDefaultSearchFormModel extends net.ibizsys.paas.ctrlmodel.SearchFormModelBase {

    public WFUserGroupDefaultSearchFormModel() {
        super();
    ...
    protected void prepareFormItems()throws Exception {
        ...
        iFormItem = this.createFormItem("n_wfusergroupname_like");
        if(iFormItem==null) {
            SearchFormItemModel formItem = new SearchFormItemModel();
            formItem.setForm(this);
            formItem.setName("n_wfusergroupname_like");
            formItem.setDEFName("WFUSERGROUPNAME");
            formItem.setCaption("用户组名称(%)");
            //设置数据项参数
            DataItemModel dataItem = new DataItemModel();
            dataItem.setName("n_wfusergroupname_like");
