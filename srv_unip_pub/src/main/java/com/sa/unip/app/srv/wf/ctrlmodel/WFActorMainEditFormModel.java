...
/**
 * 实体[工作流操作者]编辑表单[form]部件模型
 */
public class WFActorMainEditFormModel extends net.ibizsys.paas.ctrlmodel.EditFormModelBase {

    public WFActorMainEditFormModel()  {
        super();
        this.setName("form");
    }
    ...
    protected void prepareFormItems()throws Exception {
        ...
        if(iFormItem==null) {
            ...
            formItem.setDEFName("WFACTORID");
            formItem.setCaption("工作流操作者标识");
        ...
        this.registerFormItem(iFormItem);
        // wfactorid
        iFormItem = this.createFormItem("wfactorid");
        if(iFormItem==null) {
            EditFormItemModel formItem = new EditFormItemModel();
            formItem.setForm(this);
            formItem.setName("wfactorid");
            formItem.setDEFName("WFACTORID");
            formItem.setCaption("工作流操作者标识");

            //设置数据项参数
            DataItemModel dataItem = new DataItemModel();
            dataItem.setName("wfactorid");
            dataItem.setDataType(25);
            dataItem.setFormat("%1$s");
            formItem.setDataItem(dataItem);
            formItem.init();
