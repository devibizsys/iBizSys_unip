...
/**
 * 实体[工作流工作列表]编辑表单[form]部件模型
 */
public class WFWorkListMainEditFormModel extends net.ibizsys.paas.ctrlmodel.EditFormModelBase {

    public WFWorkListMainEditFormModel()  {
        super();
        this.setName("form");
    }
    ...
    protected void prepareFormItems()throws Exception {
        ...
        if(iFormItem==null) {
            ...
            formItem.setDEFName("WFWORKLISTID");
            formItem.setCaption("流程工作列表标识");
        ...
        this.registerFormItem(iFormItem);
        // wfworklistid
        iFormItem = this.createFormItem("wfworklistid");
        if(iFormItem==null) {
            EditFormItemModel formItem = new EditFormItemModel();
            formItem.setForm(this);
            formItem.setName("wfworklistid");
            formItem.setDEFName("WFWORKLISTID");
            formItem.setCaption("流程工作列表标识");

            //设置数据项参数
            DataItemModel dataItem = new DataItemModel();
            dataItem.setName("wfworklistid");
            dataItem.setDataType(25);
            dataItem.setFormat("%1$s");
            formItem.setDataItem(dataItem);
            formItem.init();
