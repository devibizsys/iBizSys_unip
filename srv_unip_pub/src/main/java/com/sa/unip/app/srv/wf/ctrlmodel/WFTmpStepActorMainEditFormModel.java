...
/**
 * 实体[工作流步骤操作者（临时）]编辑表单[form]部件模型
 */
public class WFTmpStepActorMainEditFormModel extends net.ibizsys.paas.ctrlmodel.EditFormModelBase {

    public WFTmpStepActorMainEditFormModel()  {
        super();
        this.setName("form");
    }
    ...
    protected void prepareFormItems()throws Exception {
        ...
        if(iFormItem==null) {
            ...
            formItem.setDEFName("WFTMPSTEPACTORID");
            formItem.setCaption("工作流步骤操作者（临时）标识");
        ...
        if(iFormItem==null) {
            ...
            formItem.setDEFName("WFTMPSTEPACTORNAME");
            formItem.setCaption("工作流步骤操作者（临时）名称");
        ...
        this.registerFormItem(iFormItem);
        // wftmpstepactorid
        iFormItem = this.createFormItem("wftmpstepactorid");
        if(iFormItem==null) {
            EditFormItemModel formItem = new EditFormItemModel();
            formItem.setForm(this);
            formItem.setName("wftmpstepactorid");
            formItem.setDEFName("WFTMPSTEPACTORID");
            formItem.setCaption("工作流步骤操作者（临时）标识");

            //设置数据项参数
            DataItemModel dataItem = new DataItemModel();
            dataItem.setName("wftmpstepactorid");
            dataItem.setDataType(25);
            dataItem.setFormat("%1$s");
            formItem.setDataItem(dataItem);
            formItem.init();
