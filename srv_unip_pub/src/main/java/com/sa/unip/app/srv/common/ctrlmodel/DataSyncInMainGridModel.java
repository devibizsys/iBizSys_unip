...
/**
 * 实体[数据同步接收队列]数据表格[grid]部件模型
 */
public class  DataSyncInMainGridModel extends net.ibizsys.paas.ctrlmodel.GridModelBase {

    public DataSyncInMainGridModel()  {
        super();
        this.setName("grid");
    }
    ...
    protected void prepareGridColumnModels()throws Exception {
        ...
        //建立表格列 数据同步接收队列名称
        iGridColumn = createGridColumn("datasyncinname");
        if(iGridColumn==null) {
            GridColumnModel gridColumnModel = new GridColumnModel();
            gridColumnModel.setName("datasyncinname");
            gridColumnModel.setDataItemName("datasyncinname");
            gridColumnModel.setCaption("数据同步接收队列名称");
    ...
    protected void prepareGridDataItemModels()throws Exception {
        ...
        this.registerGridDataItem(iGridDataItem);
        //建立数据项 datasyncinid
        iGridDataItem = createGridDataItem("datasyncinid");
        if(iGridDataItem == null) {
            GridDataItemModel gridDataItemModel = new GridDataItemModel();
            gridDataItemModel.setName("datasyncinid");
            DataItemParamModel  dataItemParam0 = new DataItemParamModel();
            dataItemParam0.setName("DATASYNCINID");
            dataItemParam0.setFormat("%1$s");
            dataItemParam0.setDataItem(gridDataItemModel);
            gridDataItemModel.addDataItemParam(dataItemParam0);
            gridDataItemModel.init(this);
        ...
        this.registerGridDataItem(iGridDataItem);
        //建立数据项 datasyncinname
        iGridDataItem = createGridDataItem("datasyncinname");
        if(iGridDataItem == null) {
            GridDataItemModel gridDataItemModel = new GridDataItemModel();
            gridDataItemModel.setName("datasyncinname");
            DataItemParamModel  dataItemParam0 = new DataItemParamModel();
            dataItemParam0.setName("DATASYNCINNAME");
            dataItemParam0.setFormat("%1$s");
            dataItemParam0.setDataItem(gridDataItemModel);
            gridDataItemModel.addDataItemParam(dataItemParam0);
            gridDataItemModel.init(this);
    ...
    protected void prepareGridEditItemModels()throws Exception {
        ...
        if(iGridEditItem==null) {
            ...
            gridEditItem.setDEFName("DATASYNCINID");
            gridEditItem.setEnableCond(0);
            gridEditItem.setCaption("数据同步接收队列标识");
