...
/**
 * 实体[数据同步接收队列2]数据表格[grid]部件模型
 */
public class  DataSyncIn2MainGridModel extends net.ibizsys.paas.ctrlmodel.GridModelBase {

    public DataSyncIn2MainGridModel()  {
        super();
        this.setName("grid");
    }
    ...
    protected void prepareGridColumnModels()throws Exception {
        ...
        //建立表格列 数据同步接收队列2名称
        iGridColumn = createGridColumn("datasyncin2name");
        if(iGridColumn==null) {
            GridColumnModel gridColumnModel = new GridColumnModel();
            gridColumnModel.setName("datasyncin2name");
            gridColumnModel.setDataItemName("datasyncin2name");
            gridColumnModel.setCaption("数据同步接收队列2名称");
    ...
    protected void prepareGridDataItemModels()throws Exception {
        ...
        //建立数据项 datasyncin2id
        iGridDataItem = createGridDataItem("datasyncin2id");
        if(iGridDataItem == null) {
            GridDataItemModel gridDataItemModel = new GridDataItemModel();
            gridDataItemModel.setName("datasyncin2id");
            DataItemParamModel  dataItemParam0 = new DataItemParamModel();
        ...
        this.registerGridDataItem(iGridDataItem);
        //建立数据项 datasyncin2name
        iGridDataItem = createGridDataItem("datasyncin2name");
        if(iGridDataItem == null) {
            GridDataItemModel gridDataItemModel = new GridDataItemModel();
            gridDataItemModel.setName("datasyncin2name");
            DataItemParamModel  dataItemParam0 = new DataItemParamModel();
    ...
    protected void prepareGridEditItemModels()throws Exception {
        ...
        if(iGridEditItem==null) {
            ...
            gridEditItem.setDEFName("DATASYNCIN2ID");
            gridEditItem.setEnableCond(0);
            gridEditItem.setCaption("数据同步接收队列2标识");
