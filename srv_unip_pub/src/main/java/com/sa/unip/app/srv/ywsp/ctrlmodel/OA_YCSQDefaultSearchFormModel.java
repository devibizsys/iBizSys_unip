...
/**
 * 实体[用餐申请]搜索表单[searchform]部件模型
 */
public class  OA_YCSQDefaultSearchFormModel extends net.ibizsys.paas.ctrlmodel.SearchFormModelBase {

    public OA_YCSQDefaultSearchFormModel() {
        super();
    ...
    protected void prepareFormItems()throws Exception {
        ...
        // n_oa_ycsqname_like
        iFormItem = this.createFormItem("n_oa_ycsqname_like");
        if(iFormItem==null) {
            SearchFormItemModel formItem = new SearchFormItemModel();
            formItem.setForm(this);
            formItem.setName("n_oa_ycsqname_like");
            formItem.setDEFName("OA_YCSQNAME");
            formItem.setCaption("用餐申请名称(%)");
            //设置数据项参数
            DataItemModel dataItem = new DataItemModel();
            dataItem.setName("n_oa_ycsqname_like");
            dataItem.setDataType(25);
            dataItem.setFormat("%1$s");
            formItem.setDataItem(dataItem);
