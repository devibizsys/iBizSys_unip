...
/**
 * 实体[工作流步骤操作者（临时）]数据表格[grid]部件模型
 */
public class  WFTmpStepActorMainGridModel extends net.ibizsys.paas.ctrlmodel.GridModelBase {

    public WFTmpStepActorMainGridModel()  {
        super();
        this.setName("grid");
    }

    private net.ibizsys.psrt.srv.wf.demodel.WFTmpStepActorDEModel wFTmpStepActorDEModel;
    protected  net.ibizsys.psrt.srv.wf.demodel.WFTmpStepActorDEModel getWFTmpStepActorDEModel() {
    ...
    protected void prepareGridDataItemModels()throws Exception {
        ...
        //建立数据项 wfactorid
        iGridDataItem = createGridDataItem("wfactorid");
        if(iGridDataItem == null) {
            GridDataItemModel gridDataItemModel = new GridDataItemModel();
            gridDataItemModel.setName("wfactorid");
        ...
        //建立数据项 wftmpstepactorid
        iGridDataItem = createGridDataItem("wftmpstepactorid");
        if(iGridDataItem == null) {
            GridDataItemModel gridDataItemModel = new GridDataItemModel();
            gridDataItemModel.setName("wftmpstepactorid");
            DataItemParamModel  dataItemParam0 = new DataItemParamModel();
            dataItemParam0.setName("WFTMPSTEPACTORID");
            dataItemParam0.setFormat("%1$s");
            dataItemParam0.setDataItem(gridDataItemModel);
            gridDataItemModel.addDataItemParam(dataItemParam0);
            gridDataItemModel.init(this);
        ...
        this.registerGridDataItem(iGridDataItem);
        //建立数据项 prevwfstepid
        iGridDataItem = createGridDataItem("prevwfstepid");
        if(iGridDataItem == null) {
            GridDataItemModel gridDataItemModel = new GridDataItemModel();
            gridDataItemModel.setName("prevwfstepid");
            DataItemParamModel  dataItemParam0 = new DataItemParamModel();
            dataItemParam0.setName("PREVWFSTEPID");
            dataItemParam0.setFormat("%1$s");
            dataItemParam0.setDataItem(gridDataItemModel);
            gridDataItemModel.addDataItemParam(dataItemParam0);
            gridDataItemModel.init(this);
        ...
        if(iGridDataItem == null) {
            ...
            DataItemParamModel  dataItemParam0 = new DataItemParamModel();
            dataItemParam0.setName("WFTMPSTEPACTORNAME");
            dataItemParam0.setFormat("");
            dataItemParam0.setDataItem(gridDataItemModel);
            gridDataItemModel.addDataItemParam(dataItemParam0);
            gridDataItemModel.init(this);
    ...
    protected void prepareGridEditItemModels()throws Exception {
        ...
        if(iGridEditItem==null) {
            ...
            gridEditItem.setDEFName("WFTMPSTEPACTORID");
            gridEditItem.setCaption("工作流步骤操作者（临时）标识");
