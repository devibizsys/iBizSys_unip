...
/**
 * 实体[工作流用户操作]编辑表单[form]部件模型
 */
public class WFActionMainEditFormModel extends net.ibizsys.paas.ctrlmodel.EditFormModelBase {

    public WFActionMainEditFormModel()  {
        super();
        this.setName("form");
    }
    ...
    protected void prepareFormItems()throws Exception {
        ...
        if(iFormItem==null) {
            ...
            formItem.setDEFName("WFACTIONID");
            formItem.setCaption("工作流用户操作标识");
            ...
            //注册参数
            DataItemParamModel dataItemParam0 = new DataItemParamModel();
            dataItemParam0.setName("WFACTIONID");
        ...
        if(iFormItem==null) {
            ...
            //注册参数
            DataItemParamModel dataItemParam0 = new DataItemParamModel();
            dataItemParam0.setName("WFACTIONNAME");
            dataItemParam0.setFormat("%1$s");
            dataItem.addDataItemParam(dataItemParam0);
            formItem.setDataItem(dataItem);
            formItem.init();
        ...
        this.registerFormItem(iFormItem);
        // wfactionid
        iFormItem = this.createFormItem("wfactionid");
        if(iFormItem==null) {
            EditFormItemModel formItem = new EditFormItemModel();
            formItem.setForm(this);
            formItem.setName("wfactionid");
            formItem.setDEFName("WFACTIONID");
            formItem.setCaption("工作流用户操作标识");

            //设置数据项参数
            DataItemModel dataItem = new DataItemModel();
            dataItem.setName("wfactionid");
            dataItem.setDataType(25);
            dataItem.setFormat("%1$s");
            formItem.setDataItem(dataItem);
            formItem.init();
