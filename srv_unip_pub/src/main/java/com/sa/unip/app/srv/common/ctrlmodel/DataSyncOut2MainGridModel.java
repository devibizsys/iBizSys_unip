...
/**
 * 实体[数据同步发送队列2]数据表格[grid]部件模型
 */
public class  DataSyncOut2MainGridModel extends net.ibizsys.paas.ctrlmodel.GridModelBase {

    public DataSyncOut2MainGridModel()  {
        super();
        this.setName("grid");
    }

    private net.ibizsys.psrt.srv.common.demodel.DataSyncOut2DEModel dataSyncOut2DEModel;
    protected  net.ibizsys.psrt.srv.common.demodel.DataSyncOut2DEModel getDataSyncOut2DEModel() {
    ...
    protected void prepareGridColumnModels()throws Exception {
        ...
        //建立表格列 数据同步发送队列2名称
        iGridColumn = createGridColumn("datasyncout2name");
        if(iGridColumn==null) {
            GridColumnModel gridColumnModel = new GridColumnModel();
            gridColumnModel.setName("datasyncout2name");
            gridColumnModel.setDataItemName("datasyncout2name");
            gridColumnModel.setCaption("数据同步发送队列2名称");
    ...
    protected void prepareGridDataItemModels()throws Exception {
        ...
        //建立数据项 datasyncout2name
        iGridDataItem = createGridDataItem("datasyncout2name");
        if(iGridDataItem == null) {
            GridDataItemModel gridDataItemModel = new GridDataItemModel();
            gridDataItemModel.setName("datasyncout2name");
            DataItemParamModel  dataItemParam0 = new DataItemParamModel();
            dataItemParam0.setName("DATASYNCOUT2NAME");
        ...
        this.registerGridDataItem(iGridDataItem);
        //建立数据项 datasyncout2id
        iGridDataItem = createGridDataItem("datasyncout2id");
        if(iGridDataItem == null) {
            GridDataItemModel gridDataItemModel = new GridDataItemModel();
            gridDataItemModel.setName("datasyncout2id");
            DataItemParamModel  dataItemParam0 = new DataItemParamModel();
            dataItemParam0.setName("DATASYNCOUT2ID");
            dataItemParam0.setFormat("%1$s");
            dataItemParam0.setDataItem(gridDataItemModel);
            gridDataItemModel.addDataItemParam(dataItemParam0);
            gridDataItemModel.init(this);
    ...
    protected void prepareGridEditItemModels()throws Exception {
        ...
        if(iGridEditItem==null) {
            ...
            gridEditItem.setDEFName("DATASYNCOUT2ID");
            gridEditItem.setEnableCond(0);
            gridEditItem.setCaption("数据同步发送队列2标识");
