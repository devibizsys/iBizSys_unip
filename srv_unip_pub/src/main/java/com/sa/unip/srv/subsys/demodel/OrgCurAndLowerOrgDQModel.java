...
@DEDataQuery(id="4660AA94-AAF9-40A6-9B9A-B26C35F618A7",name="CurAndLowerOrg")
@DEDataQueryCodes({
    ...
    conds={
        @DEDataQueryCodeCond(condition="( t1.`ORGID` LIKE  CONCAT(${srfsessioncontext('PRESJCODE','{\"defname\":\"ORGID\",\"dename\":\"ORG\"}')},'%') )")
    }),
    ...
    conds={
        @DEDataQueryCodeCond(condition="( t1.ORGID LIKE  ${srfsessioncontext('PRESJCODE','{\"defname\":\"ORGID\",\"dename\":\"ORG\"}')} ||'%' )")
    })
})

public class OrgCurAndLowerOrgDQModel extends net.ibizsys.paas.demodel.DEDataQueryModelBase {

    public OrgCurAndLowerOrgDQModel() {
        super();

        this.initAnnotation(OrgCurAndLowerOrgDQModel.class);
    }
