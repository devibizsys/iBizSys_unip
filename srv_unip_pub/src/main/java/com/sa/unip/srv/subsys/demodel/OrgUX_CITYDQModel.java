...
@DEDataQuery(id="EBDB781D-6A7D-442E-BF1B-CB84A6F19050",name="UX_CITY")
@DEDataQueryCodes({
    ...
    conds={
        @DEDataQueryCodeCond(condition="( t1.`PORGID` =  ${srfsessioncontext('SRFORGID','{\"defname\":\"PORGID\",\"dename\":\"ORG\"}')} )")
    }),
    @DEDataQueryCode(querycode="SELECT substr(t1.ORGID,0,4) AS ORGID, replace(replace(t1.ORGNAME,'人民检察院',''),'院','') AS ORGNAME, t1.CREATEDATE AS CREATEDATE, t1.CREATEMAN AS CREATEMAN, t1.LEVELCODE AS LEVELCODE, t1.MEMO AS MEMO, t1.ORGCODE AS ORGCODE, t1.PORGID AS PORGID, t11.ORGNAME AS PORGNAME, t1.SHORTNAME AS SHORTNAME, t1.UPDATEDATE AS UPDATEDATE, t1.UPDATEMAN AS UPDATEMAN, t1.USERDATA AS USERDATA, t1.USERDATA2 AS USERDATA2 FROM T_SRFORG t1  LEFT JOIN T_SRFORG t11 ON t1.PORGID = t11.ORGID  ",querycodetemp="",declarecode="",dbtype="ORACLE",
    fieldexps={},
    conds={
        @DEDataQueryCodeCond(condition="( t1.PORGID =  ${srfsessioncontext('SRFORGID','{\"defname\":\"PORGID\",\"dename\":\"ORG\"}')} )")
...
public class OrgUX_CITYDQModel extends net.ibizsys.paas.demodel.DEDataQueryModelBase {

    public OrgUX_CITYDQModel() {
        super();

        this.initAnnotation(OrgUX_CITYDQModel.class);
    }
