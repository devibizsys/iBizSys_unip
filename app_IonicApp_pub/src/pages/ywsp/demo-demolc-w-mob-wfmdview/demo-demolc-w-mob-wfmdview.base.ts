...
import {
	mdctrlService,
 } from './demo-demolc-w-mob-wfmdview.service';

export class demoDEMOLC_W_MobWFMDViewBase extends IBizListViewController {

    mdctrl: mdctrlService;

  constructor(opt) {
    super(Object.assign(opt, { 'url': '/IonicApp/ywsp/demoDEMOLC_W_MobWFMDView.do' }));
  }
    ...
    public getEditDataView(params: any = {}): any {
       ...
       for(let i=0;i<2;i++){
          ...
          if(!editmode)
             continue;
          if(editmode=='10'){
              return { openMode:'', className:'demoDEMOLC_W10_MobWFEditView3', viewParam:params };
          }
          if(editmode=='10@1'){
              return { openMode:'', className:'demoDEMOLC_W10_MobWFEditView3', viewParam:params };
          }
  ...
  getEditMode(data){
    ...
    if(editMode&&editMode!='')
       editMode= editMode+':'+ data.demosqbz;
    else 
       editMode= data.demosqbz;
