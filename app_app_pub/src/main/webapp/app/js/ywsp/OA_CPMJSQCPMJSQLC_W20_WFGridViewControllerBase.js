var OA_CPMJSQCPMJSQLC_W20_WFGridViewControllerBase = GridViewControllerBase.extend({
...
,getEditDataView:function(arg){
       ...
       for(var i=0;i<2;i++){
          ...
          if(!editmode)
             continue;
          if(editmode=='20@1'){
              return {title: '车牌门禁申请工作流视图(车牌门禁申请流程v1:承办处领导审核)',openMode:'',height: 0,width: 0,
   		    	layout: 'fit',maximizable:true, viewurl:'/ywsp/oa_cpmjsqcpmjsqlc_w20_wfeditview3.jsp', viewparam:arg};
          }
      }
      if(true){
	   return {  title: '车牌门禁申请工作流视图(车牌门禁申请流程v1:承办处领导审核)', openMode:'', height: 0, width: 0,
...
,getDefaultCfg:function(cfg){
     var me=this;   
    var config={backendurl:cfg.appctx+'/ywsp/OA_CPMJSQCPMJSQLC_W20_WFGridView.do?'+'srfcid='+cfg.containerid+'&',loaddefault:true
                ,viewurl:'/ywsp/oa_cpmjsqcpmjsqlc_w20_wfgridview.jsp?'+'srfcid='+cfg.containerid+'&'
...
,getEditMode:function(data){
    ...
    if(editMode&&editMode!='')
       editMode= editMode+':'+ data.cpmjsqbz;
    else 
       editMode= data.cpmjsqbz;
