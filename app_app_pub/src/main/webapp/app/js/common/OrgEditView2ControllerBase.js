var OrgEditView2ControllerBase = EditView2ControllerBase.extend({
...
,regControllers:function(config)
{
  var cid=config.containerid;
    if(cid!='')cid=cid+'_';
    this.regController(new OrgGridViewController({pcontroller:this,appctx:config.appctx,containerid:cid+'C4'}));
    this.regController(new OrgSectorGridViewController({pcontroller:this,appctx:config.appctx,containerid:cid+'C3'}));
    this.regController(new WFInstanceGridViewController({pcontroller:this,appctx:config.appctx,containerid:cid+'C6'}));
...
,getDefaultCfg:function(cfg){
    var me=this;
    var config={backendurl:cfg.appctx+'/common/OrgEditView2.do?'+'srfcid='+cfg.containerid+'&'
                ,viewurl:'/common/orgeditview2.jsp?'+'srfcid='+cfg.containerid+'&'
               ,ctrls:{form:{}}};
...
form.register(new IBizHiddenField({id:this.getCId2()+'Ma',name:'porgid'}));
form.register(new IBizFormGroup({id:this.getCId2()+'M4',name:'grouppanel1',titleBarCloseMode:'0',form:form}));
form.register(new IBizTextField({id:this.getCId2()+'M5',name:'orgname',allowBlank:false,form:form}));
form.register(new IBizTextField({id:this.getCId2()+'M6',name:'orgid',form:form}));
form.register(new IBizPickerField({id:this.getCId2()+'M7',name:'porgname',form:form,forceSelection:true,queryCaching:false, displayField:'text',valueField: 'value',valueItem:'porgid',minChars:2,pageSize:50
,pickupView:{view:'/common/orgfzpppickupview.jsp',title:'组织机构分组平铺选择视图'}
,ac:true
,url:this.getAppCtx()+'/common/OrgEditView2.do?SRFCTRLID=form&SRFFORMITEMID=porgname'

}));
form.register(new IBizTextField({id:this.getCId2()+'M8',name:'orgcode',form:form}));
form.register(new IBizTextField({id:this.getCId2()+'M9',name:'shortname',form:form}));
form.register(new IBizHiddenField({id:this.getCId2()+'Ma',name:'porgid',form:form}));
form.register(new IBizTextAreaField({id:this.getCId2()+'Mb',name:'memo',form:form,height:200}));
form.register(new IBizTextField({id:this.getCId2()+'Mc',name:'levelcode',form:form}));
...
,getDRItemView:function(arg){
      ...
      if(drmode=='DRITEM4'){
          return {viewurl:'/common/orggridview.jsp',embedviewid:'C4'};
      	}
      if(drmode=='DRITEM6'){
          return {viewurl:'/wf/wfinstancegridview.jsp',embedviewid:'C6'};
      	}
      if(drmode=='DRITEM1'){
