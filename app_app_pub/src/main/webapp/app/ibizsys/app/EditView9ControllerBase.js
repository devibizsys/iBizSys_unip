 ...
 * 编辑视图控制器对象
 */
var EditView9ControllerBase = MainViewControllerBase.extend({
	...
	onInit: function() {
		...
		if(me.hasHtmlElement('form')){
			...
			me.form.on(IBizEditForm.FORMSAVEERROR, function(sender, args, e) {
				me.onFormSaveError();
			}, me);
...
//		else
//			IBiz.alert('提示','数据保存成功！',1);
		this.fireEvent(EditView9ControllerBase.FORMSAVED, me, {afterformsaveaction:me.afterformsaveaction});
		me.updateViewInfo();
	},
	/**
	 * 表单保存完成
	 */
	onFormSaveError: function() {
		var me = this;
		
		this.fireEvent(EditView9ControllerBase.FORMSAVEERROR, me, {});
	},
	...
	updateViewInfo: function() {
		...
		me.calcToolbarItemState(!newdata);
		if (newdata) {
	...
	doSave: function(arg) {
		...
		if(arg && arg.afterformsaveaction)
			me.afterformsaveaction = arg.afterformsaveaction;
		me.saveData({});
	},
	doPrint: function() {
		var me = this;
	...
	doCopy: function() {
		...
		if (arg.srfsourcekey == undefined || arg.srfsourcekey == '') {
			IBiz.alert($IGM('IBIZAPP.CONFIRM.TITLE.WARN','警告'),$IGM('EDITVIEWCONTROLLER.DOCOPY.INFO','当前表单未加载数据，不能拷贝'),0);
			return;
		}
	...
	getFrontUIActionParam : function(uiaction){
		...
		if (uiaction.actiontarget == 'SINGLEKEY' || uiaction.actiontarget == 'MULTIKEY') {
			...
			return {srfkeys: keys,srfkey:keys};
		}
		return {};
	},
 ...
 * 表单保存成功
 */
EditView9ControllerBase.FORMSAVED = 'FORMSAVED';
/**
 * 表单保存错误
 */
EditView9ControllerBase.FORMSAVEERROR = 'FORMSAVEERROR';
